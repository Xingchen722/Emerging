...
public class AIService {
    ...
    /**
     * 处理用户消息并生成回复（集成数据读取和AI关键字提取）
     */
    public ChatMessage processMessageWithData(String message, String language) {
        try {
            logger.info("处理用户消息（集成数据读取）: {}", message);
            
            // 1. 使用AI提取关键字
            KeywordExtractionService.ExtractedKeywords extractedKeywords = keywordExtractionService.extractKeywords(message);
            Map<String, List<String>> keywords = convertToMap(extractedKeywords);
            logger.info("提取的关键字: {}", keywords);
            
                    // 2. 检查是否为Smart Tracking查询
                    if (extractedKeywords.hasSmartTrackingKeywords()) {
                        logger.info("检测到Smart Tracking查询，使用Smart Tracking服务处理");
                        return processSmartTrackingQuery(message, extractedKeywords, language);
                    }
                    
                    // 3. 检查是否为Team Coordination查询
                    if (extractedKeywords.hasTeamCoordinationKeywords()) {
                        logger.info("检测到Team Coordination查询，使用Team Coordination服务处理");
                        return processTeamCoordinationQuery(message, extractedKeywords, language);
                    }
            
            // 4. 根据关键字搜索数据
            List<PSAData> relevantData = dataReadingService.searchDataByKeywords(keywords);
            logger.info("找到相关数据: {} 条", relevantData.size());
            
            // 5. 使用OpenAI生成智能回复
            String aiResponse = generateAIResponseWithData(message, keywords, relevantData, language);
            ...
            if (!relevantData.isEmpty()) {
                response.setOriginalContent("基于 " + relevantData.size() + " 条相关数据生成");
            }
            ...
            return createErrorMessage("An error occurred while processing your message. Please try again later.", language);
        }
    }
     ...
     * 使用AI和数据生成智能回复
     */
    private String generateAIResponseWithData(String message, Map<String, List<String>> keywords, 
                                            List<PSAData> relevantData, String language) {
        try {
            if (useOpenAI && openAIService.isServiceAvailable()) {
                return generateOpenAIResponseWithData(message, keywords, relevantData, language);
            } else {
                return generateTemplateResponseWithData(message, keywords, relevantData, language);
            }
        } catch (Exception e) {
     ...
     * 使用OpenAI生成回复
     */
    private String generateOpenAIResponseWithData(String message, Map<String, List<String>> keywords, 
                                                List<PSAData> relevantData, String language) {
        try {
            // 构建包含数据的提示词
            StringBuilder prompt = new StringBuilder();
            prompt.append("作为PSA港口运营AI助手，请基于以下数据回答用户问题：\n\n");
            
            // 添加相关数据
            if (!relevantData.isEmpty()) {
                prompt.append("相关数据：\n");
                for (PSAData data : relevantData) {
                    prompt.append("- ").append(data.getDescription()).append("\n");
                    prompt.append("  港口：").append(data.getPortName()).append(" (").append(data.getPortCode()).append(")\n");
                    prompt.append("  类型：").append(data.getDataType()).append("\n");
            ...
            // 添加提取的关键字
            if (!keywords.isEmpty()) {
                prompt.append("提取的关键字：\n");
                keywords.forEach((key, values) -> 
                    prompt.append("- ").append(key).append(": ").append(String.join(", ", values)).append("\n"));
                prompt.append("\n");
            }
            
            prompt.append("User Question: ").append(message).append("\n\n");
            prompt.append("Please respond in ").append(language.equals("zh") ? "Chinese" : "English").append(" with the following requirements:\n");
            prompt.append("1. Analyze based on the provided data\n");
            prompt.append("2. Provide accurate and professional answers\n");
            prompt.append("3. If data is insufficient, please explain\n");
            prompt.append("4. Provide specific numbers and metrics\n");
            
            return openAIService.generateResponse(prompt.toString(), null, language);
            
        } catch (Exception e) {
            logger.error("OpenAI回复生成失败: {}", e.getMessage(), e);
            return generateTemplateResponseWithData(message, keywords, relevantData, language);
        }
    }
     ...
     * 使用模板生成回复
     */
    private String generateTemplateResponseWithData(String message, Map<String, List<String>> keywords, 
                                                  List<PSAData> relevantData, String language) {
        StringBuilder response = new StringBuilder();
        
        // 添加数据概览
        if (!relevantData.isEmpty()) {
            response.append("📊 数据概览：\n");
            response.append("找到 ").append(relevantData.size()).append(" 条相关数据\n\n");
            
            // 按数据类型分组显示
            Map<String, List<PSAData>> dataByType = relevantData.stream()
                .collect(Collectors.groupingBy(PSAData::getDataType));
            
            for (Map.Entry<String, List<PSAData>> entry : dataByType.entrySet()) {
                String dataType = entry.getKey();
                List<PSAData> dataList = entry.getValue();
                
                response.append("🔹 ").append(getDataTypeName(dataType, language)).append("：\n");
                for (PSAData data : dataList) {
                    response.append("  • ").append(data.getPortName()).append(" (")
                           .append(data.getPortCode()).append(")\n");
        ...
        // 添加分析结论
        response.append("📈 分析结论：\n");
        if (relevantData.isEmpty()) {
            response.append("未找到与您问题相关的数据，请尝试使用其他关键词查询。\n");
        } else {
            response.append("基于以上数据，为您提供以下分析：\n");
            
            // 根据数据类型提供分析
            if (keywords.containsKey("berth_time") || keywords.containsKey("泊位时间")) {
                response.append("• 泊位时间分析：各港口泊位效率存在差异，建议优化调度流程\n");
            }
            if (keywords.containsKey("carbon_savings") || keywords.containsKey("碳减排")) {
                response.append("• 碳减排成果：港口在环保方面取得显著成效\n");
            }
            if (keywords.containsKey("port_efficiency") || keywords.containsKey("港口效率")) {
                response.append("• 港口效率：整体运营效率良好，有进一步提升空间\n");
            }
        }
     ...
     * 获取数据类型的中文名称
     */
    private String getDataTypeName(String dataType, String language) {
        if (!"zh".equals(language)) {
            return dataType;
        }
        
        switch (dataType) {
            case "berth_time": return "泊位时间";
            case "carbon_savings": return "碳减排";
            case "port_efficiency": return "港口效率";
            case "cargo_tracking": return "货物追踪";
            case "vessel_scheduling": return "船舶调度";
            case "port_congestion": return "港口拥堵";
            default: return dataType;
        }
    }
    
    /**
     * 转换ExtractedKeywords为Map格式
     */
    private Map<String, List<String>> convertToMap(KeywordExtractionService.ExtractedKeywords extractedKeywords) {
        Map<String, List<String>> keywords = new HashMap<>();
        
        // 添加PSA关键词
        if (extractedKeywords.getPSAKeywords() != null && !extractedKeywords.getPSAKeywords().isEmpty()) {
            keywords.putAll(extractedKeywords.getPSAKeywords());
        }
        
        // 添加港口关键词
        if (extractedKeywords.getPortKeywords() != null && !extractedKeywords.getPortKeywords().isEmpty()) {
            keywords.put("ports", extractedKeywords.getPortKeywords());
        }
     ...
     * 处理Smart Tracking查询
     */
    private ChatMessage processSmartTrackingQuery(String message, KeywordExtractionService.ExtractedKeywords extractedKeywords, String language) {
        try {
            logger.info("处理Smart Tracking查询: {}", message);
            
            // 1. 解析查询
            SmartTrackingService.SmartTrackingQuery query = smartTrackingService.parseQuery(message);
            
            // 2. 搜索货物追踪信息
            List<com.networkinsights.ai.model.CargoTracking> cargoList = smartTrackingService.searchCargo(query);
            
            // 3. 生成回复
            String response = smartTrackingService.generateTrackingResponse(query, cargoList, language);
            ...
            logger.info("Smart Tracking查询处理完成");
            return chatMessage;
            
        } catch (Exception e) {
            logger.error("Smart Tracking查询处理失败: {}", e.getMessage(), e);
            return createErrorMessage("An error occurred while processing Smart Tracking query. Please try again later.", language);
        }
    }
    
    /**
     * 处理Team Coordination查询
     */
    private ChatMessage processTeamCoordinationQuery(String message, KeywordExtractionService.ExtractedKeywords extractedKeywords, String language) {
        try {
            logger.info("处理Team Coordination查询: {}", message);
            
            // 1. 解析查询
            TeamCoordinationService.TeamCoordinationQuery query = teamCoordinationService.parseQuery(message);
            
            // 2. 搜索团队成员
            List<com.networkinsights.ai.model.TeamMember> teamMembers = teamCoordinationService.findTeamMembers(query);
            
            // 3. 生成回复
            String response = teamCoordinationService.generateTeamCoordinationResponse(query, teamMembers, language);
            ...
            chatMessage.setTimestamp(java.time.LocalDateTime.now());
            chatMessage.setLanguage(language);
            chatMessage.setOriginalContent("Team Coordination查询结果");
            
            logger.info("Team Coordination查询处理完成");
            return chatMessage;
            
        } catch (Exception e) {
            logger.error("Team Coordination查询处理失败: {}", e.getMessage(), e);
            return createErrorMessage("An error occurred while processing Team Coordination query. Please try again later.", language);
        }
    }
}
