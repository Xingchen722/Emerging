...
public class OpenAIService {
    ...
    private String callOpenAIAPI(Map<String, Object> requestBody) throws IOException, InterruptedException {
        ...
        String apiUrl = baseUrl + "/openai/deployments/" + deploymentName + "/chat/completions?api-version=" + apiVersion;
        
        // 移除model字段，因为Azure OpenAI在URL中指定模型
        requestBody.remove("model");
        
        String requestBodyJson = objectMapper.writeValueAsString(requestBody);
        
        logger.info("调用Azure OpenAI API: {}", apiUrl);
        logger.debug("请求体: {}", requestBodyJson);
        
        HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(apiUrl))
                .header("api-key", apiKey)  // Azure OpenAI使用api-key头部
        ...
        if (response.statusCode() == 200) {
            ...
            logger.info("成功连接到Azure OpenAI API");
            ...
            throw new RuntimeException("无法解析API响应内容");
        } else {
            String errorBody = response.body();
            logger.error("Azure OpenAI API调用失败，状态码: {}, 响应: {}", response.statusCode(), 
                        errorBody.length() > 500 ? errorBody.substring(0, 500) + "..." : errorBody);
            throw new RuntimeException("API调用失败，状态码: " + response.statusCode() + ", 响应: " + errorBody);
        }
    }
